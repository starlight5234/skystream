...
class WatchPartyChatView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<WatchPartyChatView> createState() => _WatchPartyChatViewState();
}

class _WatchPartyChatViewState extends ConsumerState<WatchPartyChatView> {
  ...
  @override
  void didUpdateWidget(WatchPartyChatView oldWidget) {
  ...
  void _showDisconnectDialog() {
    final session = widget.session;
    final msg = session?.chatService.kickMessage ?? 'The host has ended the watch party.';
    final isKicked = session?.chatService.kickMessage?.toLowerCase().contains('kicked') == true;
  ...
  Future<String> _buildInviteUrl() async {
    ...
    return '${WatchPartyConfig.redirectUrl}?host=${Uri.encodeComponent(session.hostName)}&code=${Uri.encodeComponent(encryptedCode)}&passcode=${Uri.encodeComponent(session.passcode)}';
  }
  ...
  void _copyPasscode() {
    final session = widget.session;
    if (session != null && session.passcode.isNotEmpty) {
  ...
  Future<void> _leaveSessionConfirm() async {
    ...
    final leave = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        surfaceTintColor: Colors.transparent,
        title: Text(session.isHost ? 'End WatchParty?' : 'Leave WatchParty?'),
        content: Text(session.isHost
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(session.isHost ? 'End Party' : 'Leave'),
          ),
        ],
      ),
    );
  ...
  Widget _buildReconnectingBanner(ActiveWatchPartyState session) {
    if (session.isHost) return const SizedBox.shrink();
    return AnimatedBuilder(
      animation: session.chatService,
      builder: (context, _) {
        if (!session.chatService.isReconnecting) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              ...
              TextButton(
                onPressed: session.chatService.manualRetryReconnect,
  ...
  Widget _buildHeaderActions(ActiveWatchPartyState session, {required bool isCompact}) {
    return Row(
      ...
      children: [
        IconButton(
          icon: Icon(Icons.link_rounded, size: isCompact ? 18 : 22),
          padding: EdgeInsets.zero,
          constraints: isCompact ? const BoxConstraints() : const BoxConstraints(minWidth: 40, minHeight: 40),
          tooltip: 'Copy Invite Link',
          onPressed: _copyInviteLink,
        ),
        SizedBox(width: isCompact ? 12 : 8),
        IconButton(
          icon: Icon(Icons.power_settings_new_rounded, size: isCompact ? 18 : 22),
        ...
        PopupMenuButton<String>(
          surfaceTintColor: Colors.transparent,
          padding: EdgeInsets.zero,
          icon: Icon(Icons.more_vert_rounded, size: isCompact ? 18 : 22),
          onSelected: (val) {
            if (val == 'passcode') {
              _copyPasscode();
            } else if (val == 'people') {
  ...
  Widget build(BuildContext context) {
    ...
    // Full Screen Scaffold Mode (for WatchParty Screen Tab)
      ...
      return Scaffold(
        appBar: AppBar(
          leading: Navigator.canPop(context)
              ...
              : null,
          title: Text('${session.hostName}\'s Lobby'),
          actions: [
            _buildHeaderActions(session, isCompact: false),
          ],
        ),
        body: Column(
          children: [
    ...
    // Embedded Player Mode (for PlayerScreen)
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Spacer(),
                      _buildHeaderActions(session, isCompact: true),
                    ],
                  ),
                ),
                _buildReconnectingBanner(session),
