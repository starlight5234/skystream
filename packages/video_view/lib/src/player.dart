...
/// Configuration for an external subtitle track to be merged with the video.
class SubtitleTrackConfig {
  /// URL of the subtitle file (.srt, .vtt, etc.)
  final String uri;

  /// MIME type e.g. 'application/x-subrip' or 'text/vtt'
...
class VideoControllerMediaInfo {
  ...
  VideoControllerMediaInfo copyWith({
    int? duration,
    Map<String, VideoControllerAudioInfo>? audioTracks,
    Map<String, VideoControllerSubtitleInfo>? subtitleTracks,
    String? source,
    bool? isLive,
    bool? isSeekable,
    String? decoderName,
...
abstract class VideoController {
  ...
  /// The orientation of the current video.
  /// 0: 0 deg, 1: 90 deg, 2: 180 deg, 3: 270 deg.
  final orientation = VideoControllerProperty<int>(0);
  ...
  /// [headers] optional HTTP headers injected into every request (manifest + segments).
  /// [drmKey] optional ClearKey decryption key (hex).
  /// [drmKid] optional ClearKey ID (hex).
  void open(
    String source, {
  ...
  /// Open a media file with external subtitle tracks merged in.
  ///
  /// [source] is the url or local path of the media file.
  /// [headers] optional HTTP headers injected into every request.
  /// [subtitles] list of external subtitle sources to merge.
  /// [drmKey] optional ClearKey decryption key (hex).
  /// [drmKid] optional ClearKey ID (hex).
  void openWithSubtitles(
    String source, {
    Map<String, String>? headers,
    List<SubtitleTrackConfig>? subtitles,
    String? drmKey,
    String? drmKid,
  });
