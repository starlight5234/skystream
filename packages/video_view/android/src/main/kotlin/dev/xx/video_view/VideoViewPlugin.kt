...
class VideoController(
	...
	// Phase 3: Native-parity LoadControl — starts playback after 2.5s buffered (same as CS3IPlayer)
	private val exoPlayer = ExoPlayer.Builder(binding.applicationContext)
		.setLoadControl(
			DefaultLoadControl.Builder()
				.setBufferDurationsMs(
					15_000, // MIN_BUFFER_MS
					50_000, // MAX_BUFFER_MS
					2_500,  // BUFFER_FOR_PLAYBACK_MS
					5_000   // BUFFER_FOR_PLAYBACK_AFTER_REBUFFER_MS
				)
				.setBackBuffer(30_000, true) // 30s back buffer for instant small rewinds
				.build()
		)
	...
	// Phase 1: creates a DataSource.Factory with optional headers injected into every request
	private fun buildDataSourceFactory(headers: Map<String, String>? = null): DefaultHttpDataSource.Factory {
		val userAgent = headers?.get("User-Agent") ?: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"
		return DefaultHttpDataSource.Factory()
			.setUserAgent(userAgent)
			.setAllowCrossProtocolRedirects(true) // Phase 4: handle http->https redirects
	...
	// Phase 1: detect MIME type from URL extension
	private fun buildMediaItem(url: String, drmKey: String? = null, drmKid: String? = null): MediaItem {
		...
		when (ext) {
			...
			".ism/manifest" -> builder.setMimeType(MimeTypes.APPLICATION_SS)
		}
	...
	fun open(source: String, headers: Map<String, String>? = null, drmKey: String? = null, drmKid: String? = null): Any? {
		close()
		val url: String
		...
		try {
			...
			if (drmKey != null && drmKid != null) {
				val clearKeyJson = "{\"keys\":[{\"kty\":\"oct\",\"k\":\"${hexToBase64Url(drmKey)}\",\"kid\":\"${hexToBase64Url(drmKid)}\"}],\"type\":\"temporary\"}"
			...
			exoPlayer.setMediaSource(mediaSourceFactory.createMediaSource(buildMediaItem(url, drmKey, drmKid)))
			exoPlayer.prepare()
			state = 1U
			this.source = source
		} catch (e: Exception) {
	...
	// Phase 5: open with external subtitle sources merged via MergingMediaSource
	fun openWithSubtitles(
		source: String,
		...
		try {
			...
			val videoSource = mediaSourceFactory.createMediaSource(buildMediaItem(url, drmKey, drmKid))
			val subSources = subtitles.mapNotNull { sub ->
				val uri = sub["uri"] ?: return@mapNotNull null
				val mime = sub["mimeType"] ?: MimeTypes.APPLICATION_SUBRIP
			...
			if (subSources.isNotEmpty()) {
				exoPlayer.setMediaSource(
					MergingMediaSource(videoSource, *subSources.toTypedArray())
				)
			} else {
	...
	private fun loadEnd() {
		...
		if (exoPlayer.isCurrentMediaItemLive) {
			...
			if (!exoPlayer.currentTimeline.isEmpty) {
				...
				duration = if (window.durationMs == C.TIME_UNSET) 0L else window.durationMs
			} else {
				duration = 0L
			}
		} else {
			val rawDuration = exoPlayer.duration
			duration = if (rawDuration == C.TIME_UNSET) 0L else rawDuration
		}
	...
	override fun onPlayerError(error: PlaybackException) {
		...
		if (state > 0U) {
			when (error.errorCode) {
				// Phase 4: Live stream fell behind live edge — re-sync instead of closing
	...
	override fun onVideoSizeChanged(videoSize: VideoSize) {
		...
		if (state > 0U) {
			...
			// We send raw dimensions and rotation. The Dart layer will handle swapping if needed.
			eventSink?.success(mapOf(
				"event" to "videoSize",
				"orientation" to rotation / 90,
		...
		methodChannel.setMethodCallHandler { call, result ->
			when (call.method) {
				...
				"open" -> {
					...
					result.success(player?.open(source, headers, drmKey, drmKid))
				}
				"openWithSubtitles" -> {
					val player = players[call.argument<Int>("id")!!]
					val source = call.argument<String>("value")!!
					...
					result.success(player?.openWithSubtitles(source, headers, subtitles, drmKey, drmKid))
				}
				"seekTo" -> {
					val player = players[call.argument<Int>("id")!!]
